...
modify amount_debited decimal(9,2) not null;
...
(
in account_Number  bigint(20),
in amount_D decimal(9,2)
)
begin
declare transaction_successful tinyint(1) default true;
declare amount_in_account decimal(9,2);


    
start transaction;
	select account.amount
    into amount_in_account
    from account
    where account.account_number = account_Number;
    
    if amount_in_account >= amount_D then
		update account
		set account.amount = account.amount - amount_D
		where account.account_number = account_Number;

		insert into transaction(amount_debited, date_of_payment, account_number)
		values(amount_D, now(), account_Number);
	else 
    ...
    if transaction_successful = true then
			begin
				commit;
				select "Transaction Successful" as finalMessage;
			end;
		else
			begin
				rollback;
				select "Transaction Unsuccessful" as finalMessage;
			end;
		end if ;
end //
delimiter ;
